...
input_path  = r'C:\Users\Pstrrr...ICYK\Desktop\Teza\Matury\Pomoc_dla_Szymona_grouped_metrics.xlsx'
output_path = r'C:\Users\Pstrrr...ICYK\Desktop\Teza\Matury\Pomoc_dla_Szymona_summary.xlsx'
os.makedirs(os.path.dirname(output_path), exist_ok=True)

# 2) Load your grouped‐metrics file
df = pd.read_excel(input_path, engine='openpyxl')

# 3) Create gmina_type: urban=1 for "gmina miejska"/"miasto", else 0
urban = {'gmina miejska', 'miasto'}
df['gmina_type'] = df['unnamed:_4_level_0_typ_gminy'].isin(urban).astype(int)

# 4) Define which numeric columns belong to each panel
all_numeric = df.select_dtypes(include=[np.number]).columns.tolist()
std_cols   = [c for c in all_numeric if c.startswith(('std_between_','wstd_between_'))]
panelA_cols = [c for c in all_numeric if c not in std_cols]

# 5) Panel A: summary stats *including* standard deviation
panelA = df[panelA_cols].agg(['count','mean','median','min','max','std']).T

# 5a) Add share of technikum & branzowa
panelA.loc['share_technikum','mean'] = df['typ_placowki'].eq('technikum').mean()
panelA.loc['share_branzowa',  'mean'] = df['typ_placowki'].eq('branzowa').mean()
# leave other stats for those two as NaN

# 6) Panel B: only the std‐dev columns (we keep count, mean, median, min, max, std if you like)
panelB = df[std_cols].agg(['count','mean','median','min','max','std']).T

# 7) (Optional) Rename variable labels for readability
rename_map = {
    'year':               'Year',
    'gmina_type':         'Gmina Type (urban=1)',
    'avg_basic_score':    'Avg. Basic Score',
    'avg_advanced_score': 'Avg. Advanced Score',
    'share_technikum':    'Share Technikum',
    'share_branzowa':     'Share Branzowa',
    # and so on...
}
panelA = panelA.rename(index=rename_map)
panelB = panelB.rename(index=rename_map)

# 8) Write both panels to an Excel workbook
with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
    panelA.to_excel(writer, sheet_name='Panel A – Variables')
    panelB.to_excel(writer, sheet_name='Panel B – Std Devs')

print(" Summary (with std_dev) saved to:", output_path)
